...
const passportUploadSchema = z.object({
  file: z
...
type PassportUploadFormValues = z.infer<typeof passportUploadSchema>;

export default function PassportUpload() {
  const form = useForm<PassportUploadFormValues>({
    resolver: zodResolver(passportUploadSchema),
    defaultValues: {
      file: null,
    },
  });
  ...
  const handleNext = (values: PassportUploadFormValues) => {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleNext)} className="space-y-4">
            <FormField
              ...
              render={() => (
                <FormItem>
                  <FormLabel className="block text-sm font-black">Upload Passport</FormLabel>
                  <FormControl>
                    <div
                      {...getRootProps()}
                      className={`
